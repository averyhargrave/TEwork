<template>
  <div class="review" v-bind:class="{ favorited: review.favorited }" v-bind:key="review.id">
    ...
    <p>
      ...
      <!-- add a change event to the checkbox to run the onFavoritedChange method when clicked -->
      <input
        type="checkbox"
...
export default {
  ...
  methods: {
    onFavoritedChange() { // toggle the boolean called favorited in the datastore
      this.$state.commit("FLIP_FAVORITED", this.review)
    }
  }
};
</script>
