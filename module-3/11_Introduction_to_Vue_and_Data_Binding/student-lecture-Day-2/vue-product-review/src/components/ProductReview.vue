<template>      <!-- Our html goes here -->
    <div class='main'> <!-- Place our code in the main part of this page -->
      ...
      <!-- We want to go through the array of reviews in our data and display the attributes for each review -->
      <!-- Use a v-for loop to go through the array:  v-for="elem-name in arr-name" v-bind:key="key-field"   -->
      <!-- The key-field is used by v-for to uniquely identify an instance on the page                       -->
      ...
      > <!-- This will hold the display of the produce review data -->
        <h4>{{aReview.reviewer}}</h4>  <!-- reviewer attribute from the array -->
        <!-- generate one star image for each value rating using v-for                            -->
        ...
        <h3>{{aReview.title}}</h3>     <!-- title attribute from the array    -->
        
        <p>{{aReview.review}}</p>      <!-- review attribute from the array   -->
        
        <!-- add a checkbox tied to the favorited attribute in the array                            -->
        <!-- v-model ties a piece of data in the component to an element in the html                -->
        <!-- such that any change to the data on the page is reflected in the data of the component -->
        <!-- this is called two-way binding                                                         -->
        <p>Favorite: <input type='checkbox' v-model='aReview.favorited'/></p>
      </div> 
    </div>
</template>
...
export default {  // Expose data from this code to external processes (like Vue)
    ...
    computed: { // Stuff in here will be computed every time a value changes in the data
      ...
      // Define a computed value to determine the avg of all star-ratings
      averageRating() {
        ...
        }, 0); // initialize totalRating and make it a number
        return sum / this.reviews.length;
      },

      // Define a computed value to determine the number of one-star reviews in the reviews array
      numberOfOneStarReviews() {
        // use .reduce array function to go through the array and find the 1-star reviews
        // .reduce() takes one argument: a function that recieves two arguments (reducer, anElement)
        //    reducer is the variable the array is reduced to - numOfReviews in this example
