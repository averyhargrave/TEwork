  ...
  <div class="main">
    ...
    <div class="well-display">
      <div class="well">
        ...
        <!-- syntax:      v-on:event="what-you-want-to-do"                                  -->
    ...
    <!-- add an <a> element with a placeholder href because we want to control what happens when it's clicked -->
    <!-- use v-on to add a 'click' event handler to the link to toggle showForm between true and false        -->
    <!--     if showForm is true, make it false - if showForm is false, make it true                          -->
    <!--     to toggle a boolean: !boolean-name                                                                -->
    <a id="show-form-button" href="#" v-on:click="showForm = !showForm">Toggle Form</a>
    
    <!-- v-if: add this element to the DOM (display) if the specified condition is true -->
    <!--       remove this element from the DOM if the specified condition is false     -->
    <!-- v-show: hide (false) or display (true) this element that is already in the DOM -->

    <!-- When the submit button is clicked, we want to add the data from the form to our reviews array -->
    <!-- add a v-on for a click event to run the addReview method when the button is clicked           -->

    <!-- each input item in the form is 2-way bound to an attribute in a newReview object -->
    <!-- v-model is how we connect html elements with data in the JavaScript code         -->
    <form v-show="showForm === true" v-on:submit.prevent="addReview"> <!-- show this form if showForm property is true -->
      ...
      <!-- we only want to allow a submit when we have a valid form                        -->
      <!-- not allow submit to be active until we have determined we have a valid form     -->
      <!-- add the disabled attribute to the buttom if it says the form is invalid         -->
      ...
      <!-- NOT have the cancel button submit any form data when it is clicked -->
...
export default {
  ...
  computed: { // computed properties are generated whenever data changes on screen
    ...
    // Detemine if all the newReview fields have values - return true if they do, false if not
    isFormValid() {
  ...
  methods: {  // methods are run when they are called/referenced
    ...
    numberOfReviews(reviews, starType) {
     ...
     // this method will add a review from the form to the reviews array
    ...
    addReview() {
        this.reviews.unshift(this.newReview); // add newReview to the start (unshift) of the array
      this.resetForm(); // call another method from this one
    },
    // This method will remove the form from the display and clear its fields/attributes
    resetForm() {
      this.showForm = false; // hide the form
      this.newReview = {};   // clear the new review
    }
