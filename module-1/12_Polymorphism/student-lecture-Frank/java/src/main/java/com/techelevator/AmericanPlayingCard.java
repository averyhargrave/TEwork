...
public class AmericanPlayingCard extends PlayingCard{
	// static means there is only one occurence of the variable regardless of the # of objects
	...
	//   and constructors cannot be static, we use an anonymous
	//   method (method with no name) to call initializeMaps()
	...
	static { // the method containing the call to initializeMaps
		...
		initializeMaps(); // run the method to initailizeMaps
	}
	...
	public AmericanPlayingCard(int value, String suit) {
		// a subclass must run a superclass constructor from its constructor - super()
	...
	// Since placing data in a Map requires a method call (.put call)
