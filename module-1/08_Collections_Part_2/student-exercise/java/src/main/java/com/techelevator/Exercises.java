...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		...
		if(animalName == null) { // if the animal name given is null
			return "unknown";    // return "unknown"
		}
		// Set up a Map to associate the animalName to the groupName
		// key - animal name in lower case since we want the search to be case insensitive
		// value - group name
		
		Map<String, String> animalGroups = new HashMap(); // key is a String and value is a String
		// add entries to the Map for the animal names given
		
		animalGroups.put("rhino"    , "Crash"); // note the key is in lowercase because case matters
		...
		// check to be sure the animal was in the Map and if not, set the groupName to "unknown"
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for(String letter : words) {
			startAndEnd.put(letter.charAt(0) + "", letter.charAt(letter.length()-1) + "");
		}
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		...
		for(Integer times : ints) {
			if(integerCount.containsKey(times)) {
				int num = integerCount.get(times);
				num++;
				integerCount.put(times, num);
			} else {
				integerCount.put(times, 1);
			}
		}
