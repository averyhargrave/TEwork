...
public class Exercises {
	 ...
	 */
	//                     datatype name
	public boolean firstLast6(int[] nums) { // receiving an array of ints called nums
		// look at the first and last element in the array, return true if it is 6, otherwise return false
	...
	// also works
	//   int lastElement = nums.length-1;	this stores the last element in a variable called lastElement
	...
	/*
	 ...
	 sum3([7, 0, 0]) → 7
	 */
	public int sum3(int[] nums) { // receive an array of ints called nums
		//  Sum all numbers in the array and return sum
		
		// define a variable to hold sum so it can be returned
		int sum = 0; // hold the sum of all numbers in the array
		
	// go through the array one element at a time and add it to a sum
	// since I know how many elements are in the array, a for-loop can be used to go through the array
	// number of elements in array = arrayName.length
		
		for (int i=0; i<nums.length; i++) { // the only thing that changes is the array name
			sum = sum + nums[i];              // add current elements in the array to sum
		}
	...
	public int[] maxEnd3(int[] nums) {
		...
		int[] maxValueArray = {maxValue, maxValue, maxValue};
