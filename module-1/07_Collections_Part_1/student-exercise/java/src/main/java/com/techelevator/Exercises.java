...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		// assign each element in the array to the new ArrayList
		for (int i = 0; i < stringArray.length; i++) {
			result.add(stringArray[i]); // add current element to the array list
		}
	...
	** INTERVIEW QUESTION **
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		boolean oneShort = listOne.size() < listTwo.size();
	
		int sizeOfShort = 0;
		
		if(oneShort) {
			sizeOfShort = listOne.size();
		}
		if(!oneShort) {
			sizeOfShort = listTwo.size();
		}
		...
		if(oneShort) {
			for(int i = sizeOfShort; i < listTwo.size(); i++) {
				newList.add(listTwo.get(i));
			}
		}
		else {
			for(int i = sizeOfShort; i < listOne.size(); i++) {
				newList.add(listOne.get(i));
			}
		}
		return newList;
