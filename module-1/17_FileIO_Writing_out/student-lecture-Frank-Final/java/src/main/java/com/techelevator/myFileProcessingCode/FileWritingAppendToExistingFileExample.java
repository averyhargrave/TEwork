...
public class FileWritingAppendToExistingFileExample {
	 ...
	 *                over PrintWriter and it's ability to append to an existing file
	 * @throws IOException 
	 *********************************************************************************/
	...
	public static void main(String[] args) throws IOException {  // IOExcpetion is super class of all I/O Exceptions
		...
		if (outputFile.exists()) { // if the file exists...
			String choiceLetter = ""; 
			boolean isGoodResponse = false; // to hold whether we got a good response or not from the user
											// defined outside the loop because it is needed both inside and out of the loop
			do { // Loop at least once while the condition at the end of the loop is true
				...
				choiceLetter = userResponse.substring(0,1).toUpperCase(); // take first letter of response and convert it to uppercase
																		  // another form of case insensitivity
																		  // if they type replace or Ready or RAY, it's still going to take the R
				switch (choiceLetter) { 
					case "A": 
						appendToFile = true;
						isGoodResponse = true;
					...
					case "R":
						...
						isGoodResponse = true;
						break;
					default: // If not response expected, it is not a good response
						isGoodResponse = false;
						break;
				}
			}	while (!isGoodResponse); // loop while we don't have a good response
			}
			else { // do this if the file does not exist
		...
		// at this point we have an output file defined on the disk and whether it should be appended or replaced
		...
		// Define a PrintWriter for the BufferedWriter is so we can use the same methods as we did for System.out
        ...
        timestampNow = Timestamp.valueOf(LocalDateTime.now());    // Get the current timestamp
        diskFileWriter.println("--------------------------------------------------");
