	...
	public static void main(String[] args) throws IOException {
		...
		if (!myFile.exists() || !myFile.isFile()) {
			...
			System.exit(16);   // terminate program with a return code 16
							   // When a program uses the exit() method, an optional return code
							   //      may be sent back to the operating system
							   // If the program was run from a shell script the return code can be checked
							   //      so the script may take action based on the return code
							   // The meaning of a return code value depends on the script running the program
							   // Return codes are usually a multiple of 4, tradition from the ancient days of programming
		...
		outputFile.createNewFile(); // This will destroy any existing copy of the file
