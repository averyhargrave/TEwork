...
//  access    type   name(parameters)
    ...
    */
    ...
    format-2 of if: if (logical-expression) {
                        statements to be executed if logical-expression is true
                         ...
                         statements to be executed if the logical-expression is false
                        }     
    ...
    logical-expression uses the conditional operators (&&, ||, ==, !=, >, >=, <, <=)
    */
    public boolean returnTrueFromIf() { // changed return false to return true
        if (true) {
            return true; // return terminates the function immediately
        }

        return false; // only get to this statement if the if is false
    }
    ...
    */
    public boolean returnTrueWhenGreaterThanFive(int number) {
    	...
    	// Alternate solution
    			// return number > 5; // no if is necessary if all you are doing is returning true or false
    /*
    ...
    *
    * ternary operator is shorthand for if/else. used when an if/else is not allowed.
    *
    * format: logical-expression ? value-if-true: value-if-false;
    */
    public String returnFizzIfThreeUsingTernary(int number) {
    	return number == 3 ? "Fizz": ""; // this does the same thing as number 11
    	}
