...
public class ExceptionsLecture {
	...
	public static void main(String[] args) {
		...
		try { // If the code in this block throws an Exception
			  //     let me see if I can handle it instead of terminating the program
		...
		} catch(ArrayIndexOutOfBoundsException e) { // If this exception occurs in the preceding try block, I will handle it
													// an Exception object is automatically passed to the catch block
													// an Exception object contains info about the Exception you might want to know
													// in this particular example - we are not using info in the Exception
													// This catch block named the Exception object 'e' - call it whatever you want
		...
		try {
			...
			doSomethingDangerous();  // throws an ArrayIndexOutOfBoundsException - which will be caught by the catch block in this try block
			System.out.println("See, I told you nothing would go wrong!");
		} catch(ArrayIndexOutOfBoundsException exceptionObject) { // Will catch the Exception if anything in this try block throws it 
		...
		try {
			...
			System.out.println("You worked "+overtimeHours+" hours of overtime.");
								...
								// catch(Exception) will catch any Exception that occurs, because all Exceptions are subclasses of the Exception class
		...
		} catch(IllegalArgumentException e) { // catch an Exception thrown by any code, including method calls, inside the try block
	...
	private static double calculateHotelRoomCharges(int nights, int numberOfGuests) {
		...
		if(nights < 1) { 														 // if less than minimum number of nights 
			throw new IllegalArgumentException("Minimum number of nights is 1"); // throw an Exception
		} else if(numberOfGuests < 1) { 										 // if less than minimum number of guests
	...
	private static void doSomethingDangerous() {
		...
		for(int i = 0; i < 10; i++) {
			numbers[i] = i;  // this line will throw an Exception once i reaches 5
							 // since this method does not have the code in a try block, therefore cannot catch the Exception
