...
select * from campground where park_id = (select max(park_id) from park);
...
select * from reservation where site_id in((select site_id from site where site_number = 623), (select site_id from site where site_number = 624), (select site_id from site where site_number = 625));
