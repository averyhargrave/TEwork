...
public class ProjectsCLI {
	...
	public ProjectsCLI() {
		...
		dataSource.setUrl("jdbc:postgresql://localhost:5432/apireview"); // Name of the database you want to access
...
//      Replaced the DAO call with an API call
//		List<Department> allDepartments = departmentDAO.getAllDepartments();
		
		RestTemplate callAPI = new RestTemplate();
		
		// Call the API to get the data back as an array of Department objects
		Department[] deptArray; // This is needed because RestTemplate has no easy way to return an array
		deptArray = callAPI.getForObject("http://localhost:8080/departments", Department[].class);
		// Call the method to display the departments with the deptArray converted to a List
