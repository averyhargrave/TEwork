...
public class JDBCCityDAO implements CityDAO {
	...
	public void save(City newCity) {
		// Since all the table columns are required, we need to be sure that all variables in the
		// 	  city object passed to us are not null before we try to INSERT the data into the database
		// If any of the values in the City object passed are null - throw an exception w/ a message
	...
	private long getNextCityId() {
		// Get the next value for city_id from the database manager
		SqlRowSet nextIdResult = jdbcTemplate.queryForRowSet("SELECT nextval('seq_city_id')");
		
		if (nextIdResult.next()) { 			// If a value was return from the DBM
			return nextIdResult.getLong(1); // Retrieve the SqlRowSet and return it to the caller
		} else { // If no value was returned - we have no value for the city_id - that's not good
				 // Whenever there is an application error condition we can either 1) ignore it 
			     //															       2) display an error message and contiue execution
				 //                                                                3) display an error message and terminate the program
				 //                                                             or 4) throw an exception which will terminate the program with 
				 //                                                                   the stack trace and error messages generated by the system
			     // We chose to throw an exception this time because we want the stack trace displayed so we can see how we got to the error
				 // RuntimeException is the superclass of all exceptions - a generic exception
				 // Throw a RuntimeException if you don't have a custom exception or a system exception for what is wrong
				 // When you throw an exception you may specify a message to be included with system output for the exception
